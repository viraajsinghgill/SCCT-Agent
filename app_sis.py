...
with tab_chat:
    ...
    if active_query:
        # Build conversation context from previous turns (last 5 Q&A pairs max)
        context_parts = []
        for msg in st.session_state.chat_history[-10:]:  # last 10 messages = ~5 Q&A pairs
            if msg["role"] == "user":
                context_parts.append(f"User: {msg['content']}")
            elif msg["role"] == "assistant" and msg.get("response", {}).get("success"):
                resp = msg["response"]
                context_parts.append(f"Agent: {resp.get('summary', '')}")
        ...
        with st.chat_message("assistant", avatar="👑"):
            with st.spinner("Analyzing semantic model, verifying guardrails, and querying Snowflake Cortex..."):
                response = cortex_analyst_agent.generate_and_execute(active_query, active_persona, conversation_context)
